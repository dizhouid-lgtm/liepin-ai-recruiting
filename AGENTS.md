...
**第二步:确认登录(首跑/换机/被踢的头号坑)。** 搜之前确认这台机器已登录猎聘——**你直接运行 `LIEPIN_HEADLESS=false liepin login`(PowerShell:`$env:LIEPIN_HEADLESS='false'; liepin login`),让用户扫弹出的二维码**(用户不用自己敲命令)。扫码=用户动作,子代理代劳不了。没登录就开干,liepin 会撞登录页弹有头浏览器又关掉。
