...
└── <岗位名>/
    ...
    ├── 去重台账.csv       ← 每个召回过的人(两列 `resume_id,status`,状态六值),仅去重用
...
- **去重台账** 只为查重：召回过的每个 id 自动登记为 `未精筛`，粗筛卡面一眼否的标 `粗筛不合适`，精筛后再升级其余；状态 = `未精筛 / 粗筛不合适 / 精筛不合适 / 待提报 / 候补 / 入选`。`未精筛` = 卡面过得去/没细看的干净库存。台账增删改全走 `pxb-liepin` 的 `dedup.mjs`，别手改（保 BOM/格式）。
